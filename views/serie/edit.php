...
$id = $_GET['id'] ?? null;
$item = null;
if ($id) { $item = getSerie($id); }
if (!$item && $_SERVER['REQUEST_METHOD'] !== 'POST') { header('Location: list.php'); exit(); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id && !empty($title) && !empty($platform_id) && !empty($director_id)) {
        ...
        else { $error = "Error al actualizar."; $item = getSerie($id); } // Refetch to prevent data loss on error
    } else { $error = "Todos los campos son obligatorios."; $item = getSerie($id); }
}
?>
<!DOCTYPE html>
<html lang="es">
                    ...
                    <div class="card-body p-4">
                        ...
                        <form method="POST" action="edit.php?id=<?php echo $id; ?>">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" id="title" name="title" class="form-control" value="<?php echo htmlspecialchars($item->getTitle()); ?>" required>
                            </div>
