...
class Actor
{
    ...
    public function getAll()
    {
        ...
        foreach ($query as $item) {
            $itemObject = new Actor($item['id'], $item['first_name'], $item['last_name'], $item['birth_date'], $item['nationality']);
    ...
    public function store()
    {
        ...
        // Comprobar si existe actor con mismo nombre y apellido
        $checkStmt = $mysqli->prepare("SELECT id FROM actors WHERE first_name = ? AND last_name = ?");
        ...
        $stmt = $mysqli->prepare("INSERT INTO actors (first_name, last_name, birth_date, nationality) VALUES (?, ?, ?, ?)");
    ...
    public function update()
    {
        $updated = false;
        $mysqli = DBConection::getConnection();
        $stmt = $mysqli->prepare("UPDATE actors SET first_name = ?, last_name = ?, birth_date = ?, nationality = ? WHERE id = ?");
