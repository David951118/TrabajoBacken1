...
class SeriesAudioLanguage
{
    ...
    public function getAll()
    {
        ...
        $query = $mysqli->query('SELECT * FROM series_audio_languages');
        $listData = [];
        foreach ($query as $item) {
            $itemObject = new SeriesAudioLanguage($item['series_id'], $item['language_id']);
    ...
    public function store()
    {
        $created = false;
        $mysqli = DBConection::getConnection();
        $stmt = $mysqli->prepare("INSERT INTO series_audio_languages (series_id, language_id) VALUES (?, ?)");
    ...
    public function delete()
    {
        $deleted = false;
        $mysqli = DBConection::getConnection();
        $stmt = $mysqli->prepare("DELETE FROM series_audio_languages WHERE series_id = ? AND language_id = ?");
        $stmt->bind_param('ii', $this->series_id, $this->language_id);
        if ($stmt->execute()) {
            $deleted = true;
        }
